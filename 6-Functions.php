...
<?php
// Простая функция
// Функция в PHP — это блок кода, который выполняет определённую задачу и может быть вызван из других частей программы. Функции помогают:
// - Упрощать код, избегая повторений.
// - Делать программу более структурированной и читаемой.
// - Упрощать тестирование и отладку.

// Type Hinting (подсказки типов) в PHP — это возможность указать, какой тип данных ожидается в качестве аргумента функции или возвращаемого значения. 
// Это помогает улучшить читаемость кода, избежать ошибок и сделать его более предсказуемым.
// после закрывающей скобки () : указывается тип возвращаемого значения функции
function getName(string $name, $secondName = "Sozonov") : string {
    return "Привет $name !";
}
echo getName("Ivan");
...
getFIO("Sozonov", "Ivan", "Lavrentievich");
...
function status($name, $age, $gender = 'none'){
    echo "$name $age $gender";
}
$name = 'Ivan';
$age = 26;
status(age: $age, name: $name);

echo "<br>";

// Встроенные функции . По типу is_..., var_dump и т.д. Встроенные, то есть функции доступные без дополнительных модулей.
echo is_string($nickname);
echo "<br>";
var_dump(is_int($nickname));
echo "<br>";
...
echo "<br>";
var_dump(power($list, 3)); // Выведет: array(4) {[0]=>int(8) [1]=>int(27) [2]=>int(64) [3]=>int(125)}
...
// По ссылке - можно изменить переменную извне (Мы ссылаемся на переменную и изменяя её внутри функции мы изменяем её и извне)
